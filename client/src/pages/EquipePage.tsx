...
export default function EquipePage() {
  ...
  return <div className="container space-y-7 py-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
      <Card><CardHeader className="border-b"><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-amber-700" />Equipa atual</CardTitle></CardHeader><CardContent className="p-0">{membros.isLoading ? <p className="p-6 text-sm text-muted-foreground">A carregar colaboradores…</p> : membros.data?.length ? <div className="divide-y">{membros.data.map(({ membro, usuario }) => <div key={membro.id} className="flex items-center justify-between gap-4 px-6 py-4"><div className="min-w-0"><p className="truncate font-medium">{usuario.name || "Colaborador sem nome"}</p><p className="truncate text-sm text-muted-foreground">{usuario.email || "E-mail não informado"}</p></div><div className="text-right"><Badge variant={membro.papel === "proprietario" ? "default" : "secondary"}>{nomePapel(membro.papel)}</Badge><p className="mt-1 text-xs text-muted-foreground">{membro.ativo ? "Acesso ativo" : "Acesso suspenso"}</p></div></div>)}</div> : <p className="p-6 text-sm text-muted-foreground">Nenhum colaborador foi registado.</p>}</CardContent></Card>
