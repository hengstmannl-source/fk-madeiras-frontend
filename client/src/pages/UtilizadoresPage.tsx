...
import { Clipboard, Link2, Mail, RefreshCw, ShieldCheck, UserCheck, UsersRound } from "lucide-react";
  ...
  return <div className="container space-y-7 py-7">
    ...
    <Card>
      ...
      <CardContent className="p-0">
        {membros.isLoading ? <p className="p-6 text-sm text-muted-foreground">A carregar utilizadores…</p> : membrosAtivos.length ? <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-sm"><thead className="bg-muted/50 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-6 py-3 font-medium">Utilizador</th><th className="px-6 py-3 font-medium">Perfil</th><th className="px-6 py-3 font-medium">Estado</th><th className="px-6 py-3 font-medium">Último acesso</th></tr></thead><tbody className="divide-y">{membrosAtivos.map(({ membro, usuario }) => <tr key={membro.id}><td className="px-6 py-4"><p className="font-medium text-foreground">{usuario.name || "Utilizador sem nome"}</p><p className="mt-1 text-muted-foreground">{usuario.email || "E-mail não informado"}</p></td><td className="px-6 py-4"><Badge variant={membro.papel === "proprietario" ? "default" : "secondary"}>{nomesPapel[membro.papel] ?? membro.papel}</Badge></td><td className="px-6 py-4"><Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-800">Ativo</Badge></td><td className="px-6 py-4 text-muted-foreground">{formatarData(usuario.lastSignedIn, "Ainda não registado")}</td></tr>)}</tbody></table></div> : <p className="p-6 text-sm text-muted-foreground">Ainda não há utilizadores ativos nesta empresa.</p>}
      </CardContent>
    </Card>
