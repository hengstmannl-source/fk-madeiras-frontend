...
import { BarChart3, CalendarDays, CircleDollarSign, FilterX, Loader2, Search, TrendingDown, TrendingUp } from "lucide-react";
...
export default function RelatorioMargemVendasPage() {
  ...
  }, [busca, dataFinal, dataInicial, relatorio.data]);
  ...
  const limparFiltros = () => { setBusca(""); setDataInicial(""); setDataFinal(""); };
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">Acompanhe o valor líquido após frete, comissão e todas as taxas comerciais. A margem indicada é a parcela do subtotal que permanece após esses abatimentos.</p>
      </div>
    ...
    <Card className="border-border/60 shadow-sm">
      <CardContent className="p-4">
        ...
        <p className="mt-3 text-xs text-muted-foreground"><CalendarDays className="mr-1 inline h-3.5 w-3.5" />O período considera a data de criação das vendas aprovadas.</p>
      </CardContent>
    </Card>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Card className="border-border/60"><CardHeader className="pb-2"><CardDescription>Taxas adicionais</CardDescription><CardTitle className="text-lg text-rose-700 dark:text-rose-300">− {formatarMoeda.format(resumo.taxas)}</CardTitle></CardHeader></Card>
