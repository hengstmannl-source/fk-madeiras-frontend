...
function textoPeriodoReferencia(periodo: Registro | undefined) {
  if (!periodo?.inicio || !periodo?.fimExclusivo) return "janela definida de referências reais";
  const inicio = new Date(periodo.inicio);
  const fim = new Date(periodo.fimExclusivo);
  fim.setDate(fim.getDate() - 1);
  if (Number.isNaN(inicio.getTime()) || Number.isNaN(fim.getTime())) return "janela definida de referências reais";
...
export default function RentabilidadeMadeiraPage() {
  ...
  const periodoReferencia = textoPeriodoReferencia(materiaPrima?.periodoReferencia as Registro | undefined);
    ...
    {aba === "analise" && <>
      ...
      {dados && indicadores && <>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
          <Kpi titulo="Matéria-prima por m³" valor={indicadores.custosMateriaPrimaPorM3 == null ? "Não determinado" : moeda.format(numero(indicadores.custosMateriaPrimaPorM3))} descricao={`Total: ${moeda.format(numero(indicadores.custosMateriaPrima))} · real + referência`} icone={TreePine} destaque="verde" />
          <Kpi titulo="Industrial por m³" valor={indicadores.custosIndustriaisPorM3 == null ? "Não determinado" : moeda.format(numero(indicadores.custosIndustriaisPorM3))} descricao={`Total: ${moeda.format(numero(indicadores.custosIndustriais))} · centros e diesel`} icone={Factory} destaque="verde" />
          <Kpi titulo="Comercial / administrativo por m³" valor={indicadores.custosComerciaisAdministrativosPorM3 == null ? "Não determinado" : moeda.format(numero(indicadores.custosComerciaisAdministrativosPorM3))} descricao={`Total: ${moeda.format(numero(indicadores.custosComerciaisAdministrativos))} · estrutura`} icone={Building2} destaque="azul" />
          <Kpi titulo="Custo completo por m³" valor={indicadores.custoCompletoPorM3 == null ? "Não determinado" : moeda.format(numero(indicadores.custoCompletoPorM3))} descricao="Matéria-prima + industrial + comercial/administrativo" icone={CircleDollarSign} destaque="ambar" />
          ...
          <Kpi titulo="Cobertura por custo real" valor={`${numero(materiaPrima?.coberturaRastreavelPercentual).toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%`} descricao={`${volume.format(numero(materiaPrima?.volumeRastreavelM3))} m³ real · ${volume.format(numero(materiaPrima?.volumeEstimadoM3))} m³ referência`} icone={CheckCircle2} destaque="azul" />
        </div>

        <Card className="border-emerald-200 bg-emerald-50/35 dark:border-emerald-900 dark:bg-emerald-950/10">
          <CardHeader className="pb-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><CardTitle className="flex items-center gap-2 text-base"><TreePine className="h-4 w-4 text-emerald-700 dark:text-emerald-300" />Matéria-prima consumida</CardTitle><CardDescription>O custo real da tora e do frete tem prioridade. Excepcionalmente, a referência da mesma essência usa média ponderada por volume de tora + frete das entradas reais em {periodoReferencia}.</CardDescription></div><Badge variant="outline" className="w-fit border-emerald-300 bg-background/70 text-emerald-800 dark:border-emerald-800 dark:bg-background/20 dark:text-emerald-200">Cobertura por custo real: {numero(materiaPrima?.coberturaRastreavelPercentual).toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%</Badge></div></CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <ResumoMateriaPrima titulo="Custo real rastreável" valor={moeda.format(numero(materiaPrima?.custoTotalConhecido))} descricao={`${volume.format(numero(materiaPrima?.volumeRastreavelM3))} m³ com origem e frete real`} />
            <ResumoMateriaPrima titulo="Custo de referência por essência" valor={moeda.format(numero(materiaPrima?.custoTotalReferencia))} descricao={`${volume.format(numero(materiaPrima?.volumeEstimadoM3))} m³ · tora + frete`} tom="ambar" />
            <ResumoMateriaPrima titulo="Custo indisponível" valor={`${volume.format(numero(materiaPrima?.volumeSemReferenciaM3))} m³`} descricao="Sem origem nem referência comparável" tom="rose" />
            <ResumoMateriaPrima titulo="Custo de matéria-prima" valor={moeda.format(numero(materiaPrima?.custoTotalComEstimativa))} descricao="Custo real + referência identificada" />
            <ResumoMateriaPrima titulo="Cobertura com referência" valor={`${numero(materiaPrima?.coberturaComEstimativaPercentual).toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%`} descricao={`${numero(materiaPrima?.totalTorasConsumidas)} tora(s) consumida(s)`} tom="azul" />
          </CardContent>
        </Card>

        <Card className="border-border/70"><CardHeader><CardTitle className="text-base">Custeio por produção</CardTitle><CardDescription>Detalhe das toras consumidas na competência. Referências permanecem identificadas e não modificam plaquetas, romaneios ou títulos.</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Produção</TableHead><TableHead className="text-right">Toras / volume</TableHead><TableHead className="text-right">Custo real</TableHead><TableHead className="text-right">Referência</TableHead><TableHead className="text-right">Cobertura</TableHead><TableHead className="text-right">Custo por m³</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{producoesMateriaPrima.length === 0 ? <TableRow><TableCell colSpan={7} className="py-10 text-center text-sm text-muted-foreground">Nenhuma produção própria confirmada nesta competência.</TableCell></TableRow> : producoesMateriaPrima.map((producao) => { const expandida = producaoDetalhadaId === numero(producao.id); const custoReferenciaProducao = numero(producao.custoTorasEstimado) + numero(producao.freteEntradaEstimado); return <Fragment key={producao.id}><TableRow className="align-top"><TableCell><p className="font-medium text-foreground">{producao.numero}</p><p className="text-xs text-muted-foreground">{new Date(producao.dataProducao).toLocaleDateString("pt-BR")}</p></TableCell><TableCell className="text-right"><p className="font-medium text-foreground">{numero(producao.totalToras)} tora(s)</p><p className="text-xs text-muted-foreground">{volume.format(numero(producao.volumeTorasConsumidasM3))} m³</p></TableCell><TableCell className="text-right"><p className="font-medium text-emerald-700 dark:text-emerald-300">{moeda.format(numero(producao.custoTorasRastreavel) + numero(producao.freteEntradaRastreavel))}</p><p className="text-xs text-muted-foreground">tora + frete de origem</p></TableCell><TableCell className="text-right"><p className="font-medium text-amber-700 dark:text-amber-300">{moeda.format(custoReferenciaProducao)}</p><p className="text-xs text-muted-foreground">tora + frete por essência</p></TableCell><TableCell className="text-right"><Badge variant="outline" className={numero(producao.volumeSemReferenciaM3) > 0 ? "border-rose-300 bg-rose-50 text-rose-800 dark:border-rose-900 dark:bg-rose-950/20 dark:text-rose-200" : numero(producao.volumeEstimadoM3) > 0 ? "border-amber-300 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/20 dark:text-amber-200" : "border-emerald-300 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/20 dark:text-emerald-200"}>{numero(producao.coberturaRastreavelPercentual).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}% real</Badge><p className="mt-1 text-xs text-muted-foreground">{numero(producao.coberturaComEstimativaPercentual).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}% com referência</p></TableCell><TableCell className="text-right font-medium text-foreground">{producao.custoPorM3 == null ? "—" : moeda.format(numero(producao.custoPorM3))}</TableCell><TableCell><Button variant="ghost" size="icon" aria-label={expandida ? `Recolher detalhamento de ${producao.numero}` : `Ver detalhamento de ${producao.numero}`} onClick={() => setProducaoDetalhadaId(expandida ? null : numero(producao.id))}>{expandida ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</Button></TableCell></TableRow>{expandida && <TableRow className="bg-muted/20 hover:bg-muted/20"><TableCell colSpan={7} className="p-4"><div className="rounded-lg border border-border/70 bg-background"><div className="border-b border-border/70 px-3 py-2"><p className="text-sm font-medium text-foreground">Toras e referência de custo</p><p className="text-xs text-muted-foreground">Volume de peças: {volume.format(numero(producao.volumePecasM3))} m³{numero(producao.volumeAproveitamentoM3) > 0 ? ` · Aproveitamento incluído: ${volume.format(numero(producao.volumeAproveitamentoM3))} m³` : ""}</p></div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Plaqueta / essência</TableHead><TableHead className="text-right">Volume</TableHead><TableHead>Situação</TableHead><TableHead>Base</TableHead><TableHead className="text-right">Tora</TableHead><TableHead className="text-right">Frete</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{((producao.toras ?? []) as Registro[]).map((tora) => { const estimada = tora.situacao === "estimado_por_essencia"; const semReferencia = tora.situacao === "sem_referencia"; const custoToraLinha = estimada ? numero(tora.volumeM3) * numero(tora.referenciaEssencia?.valorMetroCubicoMedio) : numero(tora.custoTora); const freteLinha = estimada ? numero(tora.volumeM3) * numero(tora.referenciaEssencia?.fretePorMetroCubicoMedio) : numero(tora.freteEntrada); const total = custoToraLinha + freteLinha; return <TableRow key={tora.plaquetaId}><TableCell><p className="font-medium text-foreground">{tora.plaquetaCodigo}</p><p className="text-xs text-muted-foreground">{tora.essencia}</p></TableCell><TableCell className="text-right">{volume.format(numero(tora.volumeM3))} m³</TableCell><TableCell><Badge variant="outline" className={semReferencia ? "border-rose-300 text-rose-800 dark:border-rose-900 dark:text-rose-200" : estimada ? "border-amber-300 text-amber-800 dark:border-amber-900 dark:text-amber-200" : "border-emerald-300 text-emerald-800 dark:border-emerald-900 dark:text-emerald-200"}>{semReferencia ? "Custo indisponível" : estimada ? "Referência por essência" : "Custo real"}</Badge></TableCell><TableCell className="max-w-72 text-xs leading-5 text-muted-foreground">{semReferencia ? (tora.motivoCustoIndisponivel ?? "Não há origem nem referência da mesma essência.") : estimada ? `Média ponderada de tora + frete: ${moeda.format(numero(tora.referenciaEssencia?.valorMetroCubicoMedio))}/m³ + ${moeda.format(numero(tora.referenciaEssencia?.fretePorMetroCubicoMedio))}/m³ em ${numero(tora.referenciaEssencia?.quantidadePlaquetas)} plaqueta(s).` : `${tora.romaneioCargaNumero ?? "Romaneio de entrada"} · ${moeda.format(numero(tora.valorMetroCubico))}/m³ + ${moeda.format(numero(tora.fretePorMetroCubico))}/m³ de frete`}</TableCell><TableCell className="text-right">{semReferencia ? "—" : moeda.format(custoToraLinha)}</TableCell><TableCell className="text-right">{semReferencia ? "—" : moeda.format(freteLinha)}</TableCell><TableCell className="text-right font-medium">{semReferencia ? "—" : moeda.format(total)}</TableCell></TableRow>; })}</TableBody></Table></div></div></TableCell></TableRow>}</Fragment>; })}</TableBody></Table></div></CardContent></Card>
