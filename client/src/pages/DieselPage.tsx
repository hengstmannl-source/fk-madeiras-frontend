...
import { CircleDollarSign, Droplets, Fuel, Loader2, Plus, ReceiptText, Truck } from "lucide-react";
...
export default function DieselPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.18fr_1fr]">
      <Card className="min-w-0"><CardContent className="p-0"><div className="flex flex-col gap-2 border-b border-border p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold">Notas e pagamentos agendados</h2><p className="mt-1 text-sm text-muted-foreground">Entradas de diesel que geram lembrete no Contas a pagar.</p></div><Button size="sm" variant="outline" onClick={() => setNotaAberta(true)}>Adicionar nota</Button></div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Nota / fornecedor</TableHead><TableHead>Litros</TableHead><TableHead>Valor</TableHead><TableHead>Vencimento</TableHead><TableHead>Pagamento</TableHead></TableRow></TableHeader><TableBody>{resumo.isLoading ? <TableRow><TableCell colSpan={5} className="h-28 text-center text-muted-foreground"><Loader2 className="mx-auto size-4 animate-spin" /></TableCell></TableRow> : dados?.notas?.length ? dados.notas.map((item: any) => <TableRow key={item.id}><TableCell><p className="font-medium">{item.numeroNota ? `Nota ${item.numeroNota}` : "Nota sem número"}</p><p className="text-xs text-muted-foreground">{item.fornecedorNome}</p></TableCell><TableCell>{litros(item.litros)}</TableCell><TableCell>{dinheiro(item.valorTotal)}</TableCell><TableCell className="whitespace-nowrap">{dataBrasil(item.dataVencimento)}</TableCell><TableCell><EstadoTitulo estado={item.titulo?.estado} /></TableCell></TableRow>) : <TableRow><TableCell colSpan={5} className="h-28 text-center text-sm text-muted-foreground">Nenhuma nota de diesel registada.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
