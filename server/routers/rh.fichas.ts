...
export const fichasFinanceirasRh = router({
  configuracoes: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      const db = await bancoObrigatorio();
      const [item] = await db.select().from(configuracoesCustosRh).where(eq(configuracoesCustosRh.empresaId, ctx.empresaAtiva!.empresa.id)).limit(1);
      return item ?? {
        fgtsPercentual: "8",
  ...
  categorias: router({
    list: protectedProcedure.query(async ({ ctx }) => { const db = await bancoObrigatorio(); return db.select().from(categoriasLancamentosRh).where(eq(categoriasLancamentosRh.empresaId, ctx.empresaAtiva!.empresa.id)).orderBy(desc(categoriasLancamentosRh.ativo), asc(categoriasLancamentosRh.nome)); }),
    create: protectedProcedure.input(categoriaSchema).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const resultado = await db.insert(categoriasLancamentosRh).values({ ...input, empresaId: ctx.empresaAtiva!.empresa.id, criadoPor: ctx.user.id }); const id = Number(resultado[0].insertId); await auditar(ctx.empresaAtiva!.empresa.id, ctx.user.id, "categoria_ficha", id, "criada"); return { id }; }),
    update: protectedProcedure.input(categoriaSchema.partial().extend({ id: idSchema })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const { id, ...dados } = input; await db.update(categoriasLancamentosRh).set(dados).where(and(eq(categoriasLancamentosRh.id, id), eq(categoriasLancamentosRh.empresaId, ctx.empresaAtiva!.empresa.id))); await auditar(ctx.empresaAtiva!.empresa.id, ctx.user.id, "categoria_ficha", id, "atualizada"); return { success: true }; }),
  }),
  lancamentos: router({
    list: protectedProcedure.input(z.object({ colaboradorId: idSchema.optional(), competencia: competenciaSchema.optional(), tipo: z.enum(["credito", "debito", "pagamento"]).optional(), estado: z.enum(["pendente", "liquidado", "cancelado", "todos"]).optional() }).optional()).query(async ({ ctx, input }) => {
      const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const condicoes = [eq(lancamentosColaboradoresRh.empresaId, empresaId)];
    ...
    criar: protectedProcedure.input(lancamentoSchema).mutation(async ({ ctx, input }) => {
      const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const competencia = dataCompetencia(input.competencia); await competenciaAberta(db, empresaId, competencia); const colaborador = await garantirColaborador(db, empresaId, input.colaboradorId);
    ...
    cancelar: protectedProcedure.input(z.object({ id: idSchema, motivo: z.string().trim().min(3).max(2000) })).mutation(async ({ ctx, input }) => {
      const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const [lancamento] = await db.select().from(lancamentosColaboradoresRh).where(and(eq(lancamentosColaboradoresRh.id, input.id), eq(lancamentosColaboradoresRh.empresaId, empresaId))).limit(1);
  ...
  ficha: router({
    detalhe: protectedProcedure.input(z.object({ colaboradorId: idSchema, competencia: competenciaSchema.optional() })).query(async ({ ctx, input }) => {
      const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const colaborador = await garantirColaborador(db, empresaId, input.colaboradorId); const condicoes = [eq(lancamentosColaboradoresRh.empresaId, empresaId), eq(lancamentosColaboradoresRh.colaboradorId, input.colaboradorId)]; if (input.competencia) condicoes.push(eq(lancamentosColaboradoresRh.competencia, dataCompetencia(input.competencia)));
  ...
  competencias: router({
    list: protectedProcedure.query(async ({ ctx }) => { const db = await bancoObrigatorio(); return db.select().from(competenciasFinanceirasRh).where(eq(competenciasFinanceirasRh.empresaId, ctx.empresaAtiva!.empresa.id)).orderBy(desc(competenciasFinanceirasRh.competencia)); }),
    abrir: protectedProcedure.input(z.object({ competencia: competenciaSchema, observacoes: z.string().trim().max(4000).nullable().optional() })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const competencia = dataCompetencia(input.competencia); const [existente] = await db.select().from(competenciasFinanceirasRh).where(and(eq(competenciasFinanceirasRh.empresaId, empresaId), eq(competenciasFinanceirasRh.competencia, competencia))).limit(1); if (existente) return { id: existente.id, existente: true, estado: existente.estado }; const resultado = await db.insert(competenciasFinanceirasRh).values({ empresaId, competencia, observacoes: textoNulo(input.observacoes), criadoPor: ctx.user.id }); const id = Number(resultado[0].insertId); await auditar(empresaId, ctx.user.id, "competencia_financeira", id, "aberta", { competencia: input.competencia }); return { id, existente: false, estado: "aberta" as const }; }),
    fechar: protectedProcedure.input(z.object({ id: idSchema })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const [competencia] = await db.select().from(competenciasFinanceirasRh).where(and(eq(competenciasFinanceirasRh.id, input.id), eq(competenciasFinanceirasRh.empresaId, empresaId))).limit(1); if (!competencia) throw new TRPCError({ code: "NOT_FOUND", message: "Competência não encontrada." }); if (competencia.estado === "fechada") return { success: true, jaFechada: true }; await db.update(competenciasFinanceirasRh).set({ estado: "fechada", fechadaEm: new Date(), fechadaPor: ctx.user.id }).where(eq(competenciasFinanceirasRh.id, input.id)); await auditar(empresaId, ctx.user.id, "competencia_financeira", input.id, "fechada"); return { success: true, jaFechada: false }; }),
    reabrir: protectedProcedure.input(z.object({ id: idSchema, motivo: z.string().trim().min(3).max(2000) })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const [competencia] = await db.select().from(competenciasFinanceirasRh).where(and(eq(competenciasFinanceirasRh.id, input.id), eq(competenciasFinanceirasRh.empresaId, empresaId))).limit(1); if (!competencia) throw new TRPCError({ code: "NOT_FOUND", message: "Competência não encontrada." }); await db.update(competenciasFinanceirasRh).set({ estado: "aberta", reabertaEm: new Date(), reabertaPor: ctx.user.id, motivoReabertura: input.motivo }).where(eq(competenciasFinanceirasRh.id, input.id)); await auditar(empresaId, ctx.user.id, "competencia_financeira", input.id, "reaberta", { motivo: input.motivo }); return { success: true }; }),
    gerarSalarios: protectedProcedure.input(z.object({ competenciaId: idSchema })).mutation(async ({ ctx, input }) => {
      const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const [competencia, configuracaoExistente, colaboradores, custosEmpresa, custosIndividuais, encargos] = await Promise.all([
    ...
    salarios: protectedProcedure.input(z.object({ competenciaId: idSchema })).query(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const [linhas, colaboradores] = await Promise.all([db.select().from(salariosCompetenciasRh).where(and(eq(salariosCompetenciasRh.empresaId, empresaId), eq(salariosCompetenciasRh.competenciaId, input.competenciaId))), db.select().from(colaboradoresRh).where(eq(colaboradoresRh.empresaId, empresaId))]); const totais = linhas.reduce((soma, linha) => ({ salarioBruto: soma.salarioBruto + numero(linha.salarioBruto), fgts: soma.fgts + numero(linha.fgtsEstimado), inssPatronal: soma.inssPatronal + numero(linha.inssPatronalEstimado), outrosEncargos: soma.outrosEncargos + numero(linha.outrosEncargosEstimados), provisoes: soma.provisoes + numero(linha.provisaoDecimoTerceiro) + numero(linha.provisaoFerias) + numero(linha.provisaoTercoFerias), custoMensal: soma.custoMensal + numero(linha.custoMensalEstimado) }), { salarioBruto: 0, fgts: 0, inssPatronal: 0, outrosEncargos: 0, provisoes: 0, custoMensal: 0 }); return { linhas: linhas.map((linha) => ({ ...linha, colaboradorNome: colaboradores.find((colaborador) => colaborador.id === linha.colaboradorId)?.nome ?? "Colaborador removido" })), totais }; }),
  }),
  encargos: router({
    list: protectedProcedure.query(async ({ ctx }) => { const db = await bancoObrigatorio(); return db.select().from(encargosGerenciaisRh).where(eq(encargosGerenciaisRh.empresaId, ctx.empresaAtiva!.empresa.id)).orderBy(desc(encargosGerenciaisRh.ativo), asc(encargosGerenciaisRh.nome)); }),
    create: protectedProcedure.input(encargoSchema).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const resultado = await db.insert(encargosGerenciaisRh).values({ ...input, valor: Number(input.valor).toFixed(4), empresaId: ctx.empresaAtiva!.empresa.id, criadoPor: ctx.user.id }); const id = Number(resultado[0].insertId); await auditar(ctx.empresaAtiva!.empresa.id, ctx.user.id, "encargo_gerencial", id, "criado"); return { id }; }),
    update: protectedProcedure.input(encargoSchema.partial().extend({ id: idSchema })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const { id, valor, ...dados } = input; await db.update(encargosGerenciaisRh).set({ ...dados, ...(valor === undefined ? {} : { valor: Number(valor).toFixed(4) }) }).where(and(eq(encargosGerenciaisRh.id, id), eq(encargosGerenciaisRh.empresaId, ctx.empresaAtiva!.empresa.id))); await auditar(ctx.empresaAtiva!.empresa.id, ctx.user.id, "encargo_gerencial", id, "atualizado"); return { success: true }; }),
  }),
  painel: protectedProcedure.input(z.object({ competencia: competenciaSchema.optional() }).optional()).query(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const competencia = input?.competencia ? dataCompetencia(input.competencia) : null; const condicoes = [eq(lancamentosColaboradoresRh.empresaId, empresaId)]; if (competencia) condicoes.push(eq(lancamentosColaboradoresRh.competencia, competencia)); const [lancamentos, colaboradores, competencias] = await Promise.all([db.select().from(lancamentosColaboradoresRh).where(and(...condicoes)), db.select().from(colaboradoresRh).where(and(eq(colaboradoresRh.empresaId, empresaId), eq(colaboradoresRh.situacao, "ativo"))), db.select().from(competenciasFinanceirasRh).where(eq(competenciasFinanceirasRh.empresaId, empresaId)).orderBy(desc(competenciasFinanceirasRh.competencia))]); const resumo = resumoLancamentos(lancamentos); const saldo = resumo.creditos - resumo.debitos - resumo.pagamentos; const porColaborador = new Map<number, Array<typeof lancamentos[number]>>(); for (const lancamento of lancamentos) porColaborador.set(lancamento.colaboradorId, [...(porColaborador.get(lancamento.colaboradorId) ?? []), lancamento]); const colaboradoresComSaldo = colaboradores.map((colaborador) => { const valores = resumoLancamentos(porColaborador.get(colaborador.id) ?? []); return { ...colaborador, resumo: { ...valores, saldo: valores.creditos - valores.debitos - valores.pagamentos } }; }).filter((item) => item.resumo.saldo !== 0); return { resumo: { ...resumo, saldo }, colaboradoresComSaldo, competencias }; }),
  migracao: router({
    importarSaldosAdiantamentos: protectedProcedure.input(z.object({ competencia: competenciaSchema })).mutation(async ({ ctx, input }) => { const db = await bancoObrigatorio(); const empresaId = ctx.empresaAtiva!.empresa.id; const competencia = dataCompetencia(input.competencia); await competenciaAberta(db, empresaId, competencia); const abertos = await db.select().from(adiantamentosRh).where(and(eq(adiantamentosRh.empresaId, empresaId), eq(adiantamentosRh.estado, "aberto"))); let criados = 0; for (const adiantamento of abertos) { if (numero(adiantamento.saldoPendente) <= 0) continue; const descricao = `Saldo migrado de adiantamento #${adiantamento.id}`; const [existente] = await db.select().from(lancamentosColaboradoresRh).where(and(eq(lancamentosColaboradoresRh.empresaId, empresaId), eq(lancamentosColaboradoresRh.descricao, descricao), eq(lancamentosColaboradoresRh.origem, "migracao"))).limit(1); if (existente) continue; await db.insert(lancamentosColaboradoresRh).values({ empresaId, colaboradorId: adiantamento.colaboradorId, tipo: "debito", origem: "migracao", competencia, dataLancamento: new Date(), descricao, valor: decimal(adiantamento.saldoPendente), observacoes: "Saldo preservado da estrutura anterior de adiantamentos.", criadoPor: ctx.user.id }); criados += 1; } await auditar(empresaId, ctx.user.id, "migracao_ficha", 0, "adiantamentos_importados", { criados, competencia: input.competencia }); return { criados }; }),
  }),
});
